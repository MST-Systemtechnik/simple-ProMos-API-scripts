...
int main()
{
	...
	// Output response
	json j_get_response2 = json::parse(responseGet2.body);

	for (size_t i = 0; i < j_get_response2["get"].size(); i++)
	{
		std::cout << "Path: " << j_get_response2["get"][i]["path"] << endl;
		std::cout << "Value: " << j_get_response2["get"][i]["value"] << endl;
		std::cout << "------------------------------" << endl;
	}
